...
interface IdResponse { id: number }
...
export interface NodeResponse { header: NodeHeader }
export interface NodePropsResponse { props: Record<string, JsonValue> }

export interface VectorDTO { tag: string; dim: number; data: string }
export interface GetVectorsResponse { vectors: VectorDTO[] }

export interface Edge { id: number; src: number; dst: number }
export interface NeighborsResponse { neighbors: number[] }

export interface KnnHit { id: number; score: number }
export interface KnnResponse { hits: KnnHit[] }
...
export const createNode = async (labels?: string[]) => {
	const q = labels?.length ? `?${buildQuery({ labels: labels.join(",") })}` : "";
...
export const setNodeLabels = async (params: { id: number; add?: string[]; remove?: string[] }) => {
	const q = buildQuery({
		id: params.id,
...
// Edges -----------------------------------------------------------------------
export const addEdge = async (params: { src: number; dst: number; type: string }) => {
...
export const upsertVector = async (
	...
	if ("data" in params) {
		const q = buildQuery({ id: params.id, tag: params.tag, data: params.data.join(",") });
		return getJSON<OkResponse>(`/api/upsertVector?${q}`, { method: "POST" });
	}
	const q = buildQuery({ id: params.id, tag: params.tag, data_b64: params.dataB64, dim: params.dim });
	return getJSON<OkResponse>(`/api/upsertVector?${q}`, { method: "POST" });
};
...
// Graph traversal --------------------------------------------------------------
export const neighbors = async (params: { node: number; direction?: "out" | "in" | "both"; limit?: number }) => {
	const q = buildQuery({ node: params.node, direction: params.direction, limit: params.limit });
	return getJSON<NeighborsResponse>(`/api/neighbors?${q}`);
};
...
export const getNodeWithNeighbours = async (
	...
	const [{ header }, { neighbors: neighborIds }] = await Promise.all([
		getNode(id),
		neighbors({ node: id, direction: params?.direction ?? "both", limit: params?.limit }),
	]);
	...
	const edges: Edge[] = neighborIds.map((nid) => ({ id: -1, src: id, dst: nid }));
